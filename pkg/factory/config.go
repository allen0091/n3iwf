...
type Configuration struct {
	Sid    string `yaml:"sid,omitempty"    valid:"optional"`
	Sbi    *Sbi   `yaml:"sbi,omitempty"    valid:"optional"`
	NrfUri string `yaml:"nrfUri,omitempty" valid:"optional,url"`
...
type Sbi struct {
	Scheme       string `yaml:"scheme"        valid:"required,in(http|https)"`
	RegisterIPv4 string `yaml:"registerIPv4"  valid:"required,host"` // IP that is registered at NRF.
	BindingIPv4  string `yaml:"bindingIPv4"   valid:"required,host"` // IP used to run the server in the node.
	Port         int    `yaml:"port"          valid:"required,port"`
